...
import com.facultyLogin.facademicsHandler.Secure;
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		loginMenu.setPositiveButton("Save", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				{
					if(pass.equals("")){
						FacAdemicsAppClass.initialPrefs.SavePreferences(FacAdemicsAppClass.sKeyA, 0);
						FacAdemicsAppClass.initialPrefs.SavePreferences(FacAdemicsAppClass.valueKey, 1);
						FacAdemicsAppClass.loginPref.SavePreferences(FacAdemicsAppClass.idkey, id);
						changeFrag();
						nwState = false;
					}else{
						if(!pass.equals(FacAdemicsAppClass.loginPref.GetPreferences(FacAdemicsAppClass.passkey))){
							FacAdemicsAppClass.initialPrefs.SavePreferences(FacAdemicsAppClass.sKeyA, 0);
						}
				...
				if(nwState== true)
					...
					if(FacAdemicsAppClass.initialPrefs.GetPreferences(FacAdemicsAppClass.sKeyA)==1)
						new LogAuth(getSherlockActivity(), getFragmentManager());
					else{
						new Secure(getSherlockActivity(), getFragmentManager(), pass);
					}	
				else
					changeFrag();
